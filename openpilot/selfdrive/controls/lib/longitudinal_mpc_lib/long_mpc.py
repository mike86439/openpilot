...
class LongitudinalMpc:
  def __init__(self, dt=DT_MDL):
    ...
    self.nrdr = NrdrLongitudinalMpc(
      dt, T_IDXS, T_DIFFS, CRUISE_MIN_ACCEL, 2.0, MIN_X_LEAD_FACTOR, ACCEL_MIN, cloudlog.warning,
    )
  ...
  def update(self, radarstate, v_cruise, personality=log.LongitudinalPersonality.standard, model=None):
    ...
    if model is not None:
      result = self.nrdr.prepare(v_cruise, model, radarstate, personality, v_ego, self.nrdr.t_follow(personality, t_follow))
      lead_xv_0, lead_xv_1 = result.lead_0, result.lead_1
      self.status = result.status
      t_follow = result.t_follow
      x_obstacles = result.obstacles
